...
export default function MeetingCard({
  ...
  const [editedDateTime, setEditedDateTime] = useState(
    formatDateTimeLocal(new Date(meeting.date)),
  );
  ...
  const myAttendance = meeting.attendance?.find(
  ...
  return (
    <>
      <div
        ...
        className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex justify-between items-center cursor-pointer hover:border-blue-200 hover:shadow-md transition-all"
      >
        <div className="flex gap-4 items-center">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 flex items-center gap-1 mt-1">
              ...
              {meetingDate.toLocaleDateString("cs-CZ")} (
              {meetingDate.toLocaleTimeString("cs-CZ", {
                hour: "2-digit",
                minute: "2-digit",
              })}{" "}
              - {meeting.endTime})
          ...
          {canManage && (
            <>
              ...
              >
                ...
                <button
                  type="submit"
                  aria-label={`Smazat schůzku ${meeting.title}`}
          ...
          >
            ...
            <div className="flex border-b border-gray-100 px-6">
              <button
                ...
                className={`py-3 px-4 text-sm font-bold border-b-2 flex gap-2 ${activeTab === "info" ? "border-[#1a237e] text-[#1a237e]" : "border-transparent text-gray-400"}`}
              >
                <Users className="w-4 h-4" /> Účast
              </button>
              <button
                onClick={() => setActiveTab("chat")}
                className={`py-3 px-4 text-sm font-bold border-b-2 flex gap-2 ${activeTab === "chat" ? "border-[#1a237e] text-[#1a237e]" : "border-transparent text-gray-400"}`}
              >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <select
                        name="patrolId"
                        required
                        value={editedPatrolId}
                        onChange={(e) => setEditedPatrolId(e.target.value)}
                        className={inputClassName}
                      >
                    ...
                    <div className="flex gap-3 pt-2">
                      ...
                      <button
                        type="submit"
                        className="flex-1 bg-[#1a237e] hover:bg-blue-900 text-white py-3 rounded-xl font-bold transition-colors"
                      >
                        Uložit změny
                      </button>
                    </div>
                    ...
                    <div className="bg-blue-50/50 p-5 rounded-2xl border border-blue-100">
                      ...
                      {isLocked ? (
                        <div className="bg-red-50 text-red-700 p-4 rounded-xl border border-red-200 text-sm font-medium">
                          Změna účasti je uzamčena (méně než 2 hodiny do
                          začátku). Tvoje finální rozhodnutí:{" "}
                        ...
                        <form action={saveAttendance} className="space-y-4">
                          ...
                          <input
                            type="hidden"
                            name="status"
                            value={status || myAttendance?.status || ""}
                          />
                          ...
                          <div className="flex gap-3">
                            <button
                              ...
                              className={`flex-1 py-3 rounded-xl font-bold border-2 flex items-center justify-center gap-2 ${(status || myAttendance?.status) === "GOING" ? "border-green-500 bg-green-50 text-green-600" : "border-gray-200 text-gray-400"}`}
                            >
                              <CheckCircle2 className="w-5 h-5" /> Přijdu
                            </button>
                            <button
                              type="button"
                              onClick={() => setStatus("NOT_GOING")}
                              className={`flex-1 py-3 rounded-xl font-bold border-2 flex items-center justify-center gap-2 ${(status || myAttendance?.status) === "NOT_GOING" ? "border-red-500 bg-red-50 text-red-600" : "border-gray-200 text-gray-400"}`}
                            >
                              <XCircle className="w-5 h-5" /> Nepřijdu
                            </button>
                          </div>
                      ...
                      <div className="mt-6 border-t pt-6">
                        ...
                        {meeting.attendance
                          ...
                          .map((a: any) => (
                            <div
                              key={a.id}
                              className="bg-red-50 p-3 rounded-lg mb-2 text-sm border border-red-100"
                            >
                ...
                <div className="flex flex-col h-[400px]">
                  <div className="flex-1 overflow-y-auto space-y-4 pr-2">
                    {meeting.messages.length === 0 ? (
                      <p className="text-gray-400 text-center text-sm italic mt-10">
                        Zatím tu nejsou žádné zprávy. Napiš jako první!
                      </p>
                    ) : (
                      meeting.messages.map((msg: any) => (
                        ...
                        >
                          ...
                          <div
                            className={`px-4 py-2 rounded-2xl max-w-[85%] text-sm ${msg.userId === currentUser.id ? "bg-[#1a237e] text-white rounded-tr-none" : "bg-gray-100 text-gray-800 rounded-tl-none"}`}
                          >
                            {msg.text}
                          </div>
                        </div>
                  ...
                  <form action={sendMessage} className="mt-4 flex gap-2">
                    ...
                    <input
                      ...
                      name="text"
                      required
                      placeholder="Napiš zprávu družině..."
                      className={`${inputClassName} flex-1`}
                    />
                    <button
                      type="submit"
                      aria-label="Odeslat zprávu"
                      className="p-3 bg-[#00c853] text-white rounded-xl hover:bg-green-600 transition-colors"
                    >
