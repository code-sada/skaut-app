...
import { Pencil, Trash2, MapPin, CalendarDays, Clock, Users, Info, X, Banknote, Tent, CheckCircle2, XCircle } from 'lucide-react'

// Přidány props: currentUser a saveAttendanceAction
export default function EventCard({ event, deleteEventAction, canManage, currentUser, saveAttendanceAction }: { event: any, deleteEventAction: any, canManage?: boolean, currentUser?: any, saveAttendanceAction?: any }) {
  ...
  const myAttendance = event.attendance?.find((a: any) => a.userId === currentUser?.id)
  const [status, setStatus] = useState<string | null>(myAttendance?.status || null)
  const [note, setNote] = useState<string>(myAttendance?.note || '')

  // Filtrace účastníků pro záložku "Účastníci"
  const going = event.attendance?.filter((a: any) => a.status === 'GOING') || []
  const notGoing = event.attendance?.filter((a: any) => a.status === 'NOT_GOING') || []
                ...
                <div className="space-y-8">
                  ...
                  {currentUser && (
                    <div className="bg-white border-2 border-blue-100 p-5 rounded-2xl shadow-sm">
                      <h3 className="text-lg font-bold text-[#1a237e] mb-4">Tvoje účast</h3>
                        ...
                        {status && (
                          <div className="animate-in fade-in slide-in-from-top-2 duration-300 pt-2">
                            <label className="block text-sm font-bold text-gray-700 mb-1">
                              Poznámka <span className="font-normal text-gray-400">(alergie, léky, dřívější odjezd...)</span>
                            </label>
                            <textarea 
                              name="note"
                              value={note}
                            ...
                            />
                            <button type="submit" className="w-full mt-3 bg-blue-600 hover:bg-blue-700 text-white p-3 rounded-xl font-bold transition-colors">
                ...
                <div className="space-y-8">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-[#00c853] mb-4 flex items-center gap-2">
                      <CheckCircle2 className="w-5 h-5" /> Přihlášení ({going.length})
                    </h3>
                    {going.length === 0 ? (
                      <p className="text-gray-500 text-sm italic">Zatím nikdo nepotvrdil účast.</p>
                    ) : (
                      <div className="space-y-3">
                        {going.map((att: any) => (
                          <div key={att.id} className="bg-white border border-green-100 p-4 rounded-xl shadow-sm">
                            <p className="font-bold text-gray-900">{att.user?.name || 'Neznámý uživatel'}</p>
                            {att.note && (
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-red-500 mb-4 flex items-center gap-2 border-t border-gray-100 pt-6">
                      <XCircle className="w-5 h-5" /> Omluvení ({notGoing.length})
                    </h3>
                    {notGoing.length === 0 ? (
                      <p className="text-gray-500 text-sm italic">Zatím se nikdo neomluvil.</p>
                    ) : (
                      <div className="space-y-3">
                        {notGoing.map((att: any) => (
                          <div key={att.id} className="bg-gray-50 border border-gray-200 p-4 rounded-xl">
                            <p className="font-bold text-gray-500 line-through">{att.user?.name || 'Neznámý uživatel'}</p>
                            {att.note && <p className="text-sm text-gray-500 mt-1">Důvod: {att.note}</p>}
                          </div>
                        ))}
                      </div>
                    )}
