...
export default async function HomePage() {
  ...
  // 4. Stažení akcí VČETNĚ účastníků a jejich jmen
  const events = await prisma.event.findMany({
    orderBy: { date: 'asc' },
    include: {
      attendance: {
        include: { 
          user: true // Tímto zajistíme, že v EventCard uvidíme jména přihlášených
        }
      }
    }
  })
      ...
      <div className="space-y-4">
        {events.map((event) => (
          <EventCard 
            ...
            canManage={canManage}
            currentUser={currentUser} // Předáváme uživatele (aby si mohl naklikat účast)
            saveAttendanceAction={saveAttendance} // Předáváme akci na uložení
          />
        ))}
